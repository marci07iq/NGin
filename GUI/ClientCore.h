...
namespace NGin {
  namespace Graphics {
    ...
    extern std::map<int, key_config> keybinds; //key, display name
    ...
    bool checkKey(int id, std::set<key_location>& down, key special = key(), bool need = false);
    bool checkKey(key id, std::set<key_location>& down);
