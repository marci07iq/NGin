...
namespace NGin {
  namespace Graphics {
    ...
    int TableRow::mouseMoved(int mx, int my, int ox, int oy, std::set<key_location>& down) {
      auto it = data.end();
    ...
    int TableRow::guiEvent(gui_event& evt, int mx, int my, std::set<key_location>& down) {
    ...
    int Table::mouseMoved(int mx, int my, int ox, int oy, std::set<key_location>& down) {
      auto it = data.end();
    ...
    int Table::guiEvent(gui_event& evt, int mx, int my, std::set<key_location>& down) {
      if (evt._key._type == key::type_wheel) {
        if (!evt.captured && isIn(mx, my)) {
          evt.captured = true;
          scroll += 30 * evt._key._keycode;
