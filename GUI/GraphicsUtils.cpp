...
namespace NGin {
  namespace Graphics {
    std::map<string, std::map<string, colorargb>> colors; //type, elem
    ...
    void resetViewport() {
      ...
      glScissor(0, 0, current->width, current->height);
      ...
      //glPushMatrix();
      //glLoadIdentity();
      //glOrtho(0, current->width,
      //  0, current->height, -1, 1);
      //glMatrixMode(GL_MODELVIEW);
      //glPushMatrix();
      //glLoadIdentity();
      //glColor3ub(0, 255, 0);
  ...
  namespace Gll {
    gllModes _mode;
    vector<pair<fVec2, colorargb>> _pts;
    colorargb _col;

    NGin::Graphics::RawWinHwnd initOn = NULL;
    ...
    iVec2 gllFontCharCount;

    stack<fVec2> _offsets;
    ...
    void gllText(string s, int x, int y, int xAlign, int yAlign, float scale) {
    ...
    PolyVao gllBuild() {
      ...
      case GLL_POLY:
        ...
        for (int i = 0; i < _pts.size() - 2; i++) {
          ...
          NGin::Graphics::insertColor(col, 12 * i, _pts[0].second);

          raw[6 * i + 2] = _pts[i + 1].first.x;
          raw[6 * i + 3] = _pts[i + 1].first.y;
          NGin::Graphics::insertColor(col, 12 * i + 4, _pts[i + 1].second);

          raw[6 * i + 4] = _pts[i + 2].first.x;
          raw[6 * i + 5] = _pts[i + 2].first.y;
          NGin::Graphics::insertColor(col, 12 * i + 8, _pts[i + 2].second);
        }
        break;
      ...
      case GLL_QUADS:
        ...
        for (int i = 0; i < _pts.size() / 4; i++) {
          ...
          NGin::Graphics::insertColor(col, 24 * i + 0, _pts[4 * i + 0].second);

          raw[12 * i + 2] = _pts[4 * i + 1].first.x;
          raw[12 * i + 3] = _pts[4 * i + 1].first.y;
          NGin::Graphics::insertColor(col, 24 * i + 4, _pts[4 * i + 1].second);

          raw[12 * i + 4] = _pts[4 * i + 2].first.x;
          raw[12 * i + 5] = _pts[4 * i + 2].first.y;
          NGin::Graphics::insertColor(col, 24 * i + 8, _pts[4 * i + 2].second);

          raw[12 * i + 6] = _pts[4 * i + 2].first.x;
          raw[12 * i + 7] = _pts[4 * i + 2].first.y;
          NGin::Graphics::insertColor(col, 24 * i + 12, _pts[4 * i + 2].second);

          raw[12 * i + 8] = _pts[4 * i + 3].first.x;
          raw[12 * i + 9] = _pts[4 * i + 3].first.y;
          NGin::Graphics::insertColor(col, 24 * i + 16, _pts[4 * i + 3].second);

          raw[12 * i + 10] = _pts[4 * i + 0].first.x;
          raw[12 * i + 11] = _pts[4 * i + 0].first.y;
          NGin::Graphics::insertColor(col, 24 * i + 20, _pts[4 * i + 0].second);
        }
        break;
        /*case GLL_LINES:d
        size = 2 * (_pts.size() / 2);
    ...
    void gllRender(PolyVao what, fVec2 center, fVec2 zoom) {

      gllBaseS->bind();

      GLint loc = glGetUniformLocation(gllBaseS->_pID, "scale");
      if (loc != -1) {
        //cout << NGin::Graphics::current->width << " " <<  << endl;
        glUniform2f(loc,
          NGin::Graphics::current->width / 2.0 * zoom.x,
          NGin::Graphics::current->height / 2.0 * zoom.y);
      } else {
